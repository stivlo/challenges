...
public class Solution {
    ...
    public int repeatedNumber(final List<Integer> a) {
        int root = (int) Math.floor(Math.sqrt(a.size()));
        int bucket = findBucket(a, root);
        return findDuplicate(a, root, bucket);
    }

    private int findBucket(List<Integer> a, int root) {
        int[] freq = new int[root + 2];
        for (int i = 0; i < a.size(); i++) {
            int index = (a.get(i) - 1) / root;
            freq[index]++;
            if (freq[index] > root) return index;
        }
    ...
    private int findDuplicate(List<Integer> a, int root, int bucket) {
        Set<Integer> nums = new HashSet<>();
        for (int i = 0; i < a.size(); i++) {
            int index = (a.get(i) - 1) / root;
            if (bucket == index) {
                if (nums.contains(a.get(i))) {
                    return a.get(i);
                }
                nums.add(a.get(i));
            }
    ...
    public static void main(String[] args) {
        assertThat(new Solution().repeatedNumber(Lists.newArrayList(3, 2, 1, 4, 5, 5)), is(5));
        assertThat(new Solution().repeatedNumber(Lists.newArrayList(3, 4, 1, 4, 1)), is(4));
        assertThat(new Solution().repeatedNumber(Lists.newArrayList(8, 1, 2, 3, 4, 5, 6, 7, 7)), is(7));
        assertThat(new Solution().repeatedNumber(Lists.newArrayList(247, 240, 303, 9, 304, 105, 44, 204, 291,
                ...
                is(73));
        assertThat(new Solution().repeatedNumber(Lists.newArrayList(127, 228, 308, 341, 350, 371, 195, 11,
